...
// tulis dulu mau apa aja

// aku mau supaya dia ketika di clickdia aktifin handlenya--->> ganti on hover aja
// aku mau supaya tombol itu ada penanda aktif apa kaganya, kl aktif baru bisa aktif fungsi yg diatas kl kaga langsungilang penanda dan gabisa aktif
// aku mau supaya ketika kita menklik div resizable yg lain yg diklik sebelumnya jadi non aktif handlenya

// objek/ input apa aja yg kamu perlukan?
// -on click tombol
// -on click div resizable?
//
...
//proses:
// -fungsi addraghandle & resize handle
// -fungsi tambahan:

var buttonStatus = false;

// Get the button element and add the event listener for 'click'
var buttonHandle = document.querySelector('.HandleButton');
buttonHandle.addEventListener('click', function() {
  // Toggle the button status when the button is clicked
  buttonStatus = !buttonStatus;

  // If buttonStatus is true, add the hover event listener to the resizable div
  if (buttonStatus === true) {
    const ResizableDiv = document.querySelector('.Resizable');
    ResizableDiv.addEventListener('mouseenter', addHandle); // Add hover (mouseenter) event listener
  }
});

// Function that will be triggered on hover (mouseenter)
function addHandle() {
  console.log('Hovered over the resizable div!');
  // Your code to handle the hover event here
}